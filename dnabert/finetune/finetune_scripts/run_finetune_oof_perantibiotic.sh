...
# paths
export FOLD_DATA_DIR=$1 #this is base dir containing each fold directory, each containing train.csv, test.csv, dev.csv
export FOLD_BASE_RUN_NAME=$2 #base run name, _{fold} will be attached
for FOLD in $(ls -d ${FOLD_DATA_DIR}/*); do
    FOLD=$(basename ${FOLD})
    echo "Running finetuning for fold: $FOLD"

    export DATA_DIR="${FOLD_DATA_DIR}/${FOLD}/dnabert_data" #this is base dir containing per antibiotic directories with train/test/dev each
    export BASE_RUN_NAME="${FOLD_BASE_RUN_NAME}_${FOLD}" #base run name, _{antibiotic} will be attached
    export BASE_OUT_DIR="/gpfs/scratch/jvaska/CAMDA_AMR/AMR_v2/dnabert/finetune/finetuned_models/${FOLD_BASE_RUN_NAME}/${FOLD}"
    ...
    for ANTIBIOTIC in TET CAZ GEN ERY; do
